...
// 
// To implement of DES cipher we have used some parts of implementation of DES coded by:
...
{
//	return (((16 * row + col) & BIT(5)) ? 32 : 0) | ((16 * row + col) & BIT(0) ? 16 : 0) | (((16 * row + col) >> 1) & 0xf);
...
{
	ctx->subkey[round-1] = 0;
	ctx->subkey[round-1] = ctx->rk[0];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 6)^ ctx->rk[1];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 6) ^ ctx->rk[2];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 6) ^ ctx->rk[3];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 6) ^ ctx->rk[4];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 6) ^ ctx->rk[5];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 6) ^ ctx->rk[6];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 6) ^ ctx->rk[7];
	ctx->subkey[round-1] = (ctx->subkey[round-1] << 16);
...
//#########################################################################################################3
void rounds_with_Fault(Fault_DES_Alg* Fault,uint64_t *data, uint64_t key)
{

int Last_DesSbox[8][4][16] = {
		{
			{ 14,  4, 13,  1,  2, 15, 11,  8,  3, 10,  6, 12,  5,  9,  0,  7 },
			{ 0, 15,  7,  4, 14,  2, 13,  1, 10,  6, 12, 11,  9,  5,  3,  8 },
			{ 4,  1, 14,  8, 13,  6,  2, 11, 15, 12,  9,  7,  3, 10,  5,  0 },
...
Last_DesSbox[Fault->Sbox][Fault->row][Fault->col] = ((DesSbox[Fault->Sbox ][Fault->row][Fault->col] +1) % 16);
//	printf("%d\n ", Last_DesSbox[0][0][0]);
	...
	//ctx->Valu_before_addingKey = right_block;
	//	printf("\n Rblock: \n %016llx \t", right_block);

	// 2. Xor with the key
	right_block = right_block ^ key;

	//uint64_t InputSbox = right_block; //0xxxx0000
	//InputSbox = ((InputSbox >> (64 - 6 * (i))) & 0x000000000000003F); //input of faulty Sbox
	...
	for (int ii = 0; ii < 8; ii++)
	{
		...
		for (int jj = 1; jj < 5; jj++)
		{
			...
			substitued = Last_DesSbox[ii][coordx][coordy];
...
void DES_Alg_EncFaulty(Fault_DES_Alg* Fault, uint64_t *data, uint64_t key)
{
	...
	for (int ii = 0; ii < 16; ii++)
	{
			rounds_with_Fault(Fault,data, a_key[ii]);
	}
	*data = (*data << 32) + (*data >> 32);
	Permutation(data, 0);
...
void DES_Alg_Dec(uint64_t *data, uint64_t key)
{
	// Key schedual
	uint64_t a_key[16];
	a_key[0] = key;
...
void DES_Expansion(Fault_DES_Alg *Fault, uint64_t *data, uint64_t *OutputExpansion) // for all rounds except last round
{
	uint64_t right_block = 0;
	...
	*OutputExpansion = right_block;

	*OutputExpansion = ((*OutputExpansion >> (64 - 6 * (Fault->Sbox + 1))) & 0x3F); //input of faulty Sbox
...
void DES_Expansion_for_lastRound(uint64_t *data, uint64_t *OutputExpansion)
{
	Permutation(data, 1); //it is added for Alg 2 and also "int Sbox" is added instead of "Fault_DES_Alg *Fault"
	uint64_t right_block = 0;
...
//	*OutputExpansion = ((*OutputExpansion >> (64 - 6 * (Sbox + 1))) & 0x3F); //input of faulty Sbox
...
uint64_t Extract_rth_6bit_from_64bit(uint64_t a, int r)
{
	return ((a >> (64 - 6 * (r + 1))) & 0x3F);
}
